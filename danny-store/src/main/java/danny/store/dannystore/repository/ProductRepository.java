...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = "select * from product p where p.name like %?1% order by p.price asc", nativeQuery = true)
    List<Product> searchProductDtosByName(String name);
    @Query(value = "select * from product p where p.name like %?1% order by p.price desc", nativeQuery = true)
